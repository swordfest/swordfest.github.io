...
window.addEventListener("load", (event) => {
  ...
  nav.style.setProperty("padding-bottom","none", "important");
	...
	for (let i = 0; i < contenido.length; i++) {
		var c = contenido[i].innerHTML;
		if (c.includes("Ejercicio") || c.includes("ejercicio")) {
			contenido[i].classList.add("ejercicio");
			// console.log(contenido[i]);
		}

		if (c.includes("Consolidación") || c.includes("consolidación")) {
			contenido[i].classList.add("consolidacion");
		}
...
var $exe = {
	init: function () {
		...
		this.setModalWindowContentSize();
		...
		$(".feedbackbutton.feedback-toggler").click(function () {
			...
			if (
				this.value == $exe_i18n.showFeedback ||
				this.value == $exe_i18n.hideFeedback
			)
				changeText = true;
		...
		$(".textIdevice,.pblIdevice").each(function (i) {
			$(".feedbackbutton", this).each(function () {
				...
				if (buttonTxt.length == 2) {
					buttonTxt = [$.trim(buttonTxt[0]), $.trim(buttonTxt[1])];
				...
				$(this).click(function () {
					...
					var hasCustomText =
						typeof window["$exeTextIdeviceButtonText" + i] != "undefined";
					if (feedback.is(":visible")) {
						if (hasCustomText)
							this.value = window["$exeTextIdeviceButtonText" + i][0];
						feedback.slideUp();
					} else {
						if (hasCustomText)
							this.value = window["$exeTextIdeviceButtonText" + i][1];
						feedback.slideDown();
					}
					return false;
				});
		...
		$("form.cloze-form").submit(function () {
			...
			try {
				...
				if ($("body").hasClass("exe-epub3"))
					txt += "<br /><br />" + $exe_i18n.epubJSerror;
		...
		if (window.DOMParser) this.clientSearch.init(bod);
	},
	clientSearch: {
		init: function (bod) {
					...
					success: function (xml) {
						...
						var sF =
							...
							<form id="exe-client-search-form" action="#" method="GET">\
								<p><label for="exe-client-search-text" class="sr-av">' +
							$exe_i18n.fullSearch +
							': </label><input type="text" id="exe-client-search-text" /> \
								<input type="submit" id="exe-client-search-submit" value="' +
							$exe_i18n.search +
							'" />\
								<a href="#main" id="exe-client-search-reset" title="' +
							$exe_i18n.hideResults +
							'"><span>' +
							$exe_i18n.hideResults +
							'</span></a></p>\
							</form>\
						...
						$exe.clientSearch.results.css(
							"min-height",
							$exe.clientSearch.main.height() + "px"
						);
						$("#skipNav").append(
							' <a href="#exe-client-search-text" id="exe-client-search-lnk" class="sr-av">' +
								$exe_i18n.fullSearch +
								"</a>"
						);
		...
		strip: function (html) {
			...
			if (
				html.indexOf('<div class="adivina-IDevice') == 0 ||
				html.indexOf('<div class="quext-IDevice') == 0 ||
				html.indexOf('<div class="rosco-IDevice') == 0 ||
				html.indexOf('<div class="vquext-IDevice') == 0
			) {
		...
		getNodeHTML: function (nodeNo, sTitle, query, html) {
			...
			$("instance", div).each(function () {
				if (
					$(this).attr("class") == "exe.engine.node.Node" ||
					$(this).attr("class") == "exe.engine.notaidevice.NotaIdevice"
				) {
					$(this).remove();
				}
			});
			...
			div.find("unicode").each(function () {
				if ($(this).attr("content") == "true") {
					...
					if (typeof currHTML == "string")
						currHTML = $exe.clientSearch.strip(currHTML);
					if (
						currTit.indexOf(query) != -1 ||
						currHTML.toLowerCase().indexOf(query) != -1
					) {
						var a = as.eq(nodeNo);
						...
						if (a.length == 1) {
							...
							if (currHTML == "") currHTML = "...";
								...
								res +=
									...
									a.attr("href") +
									'" \
							class="exe-client-search-result-link">' +
									sTitle +
									'</a> &rarr; </strong>\
							<span class="exe-client-search-result-detail">' +
									currHTML +
									"</span></li>";
						}
					}
		...
		splitByWords: function (text, startFrom, lengthFrom) {
			var len = text.length,
				...
				fr =
					startFrom <= 0
						? 0
						: text.substr(startFrom).search(re) + startFrom + 1,
				to =
					lengthFrom >= len
						? len
						: text.substr(lengthFrom).search(re) + lengthFrom;
			if (fr === -1) fr = 0;
		...
		search: function (query) {
			...
			$(xml)
				...
				.each(function () {
					if ($(this).attr("class") == "exe.engine.node.Node") {
						...
						results += $exe.clientSearch.getNodeHTML(
							nodeNo,
							sTitle,
							query,
							str.replace(/script/g, "script_")
						);
						nodeNo++;
					}
				});
			if (results != "") {
				results =
					"<p>" +
					$exe_i18n.searchResults.replace(
						"%",
						"<strong>" + query + "</strong>"
					) +
					":</p><ul>" +
				...
				$(".exe-client-search-result-link", $exe.clientSearch.results).html(
					function (_, html) {
						html = html.replace(/script_/g, "script");
						var re = new RegExp("(" + query + ")", "gi");
						return html.replace(
							re,
							'<mark class="exe-client-search-result">$1</mark>'
						);
					}
				);
				$(".exe-client-search-result-detail", $exe.clientSearch.results).each(
					function (i) {
						var max = 200;
						var c = $(this).text();
						c = $exe.clientSearch.strip(c);
						var n = "";
						if (c.length > max + 100) {
							...
							n += start;
							n +=
								'<a href="#exe-client-search-text-' +
								i +
								'" title="' +
								$exe_i18n.more +
								'" class="exe-client-search-read-more">[&hellip;]</a>';
							n +=
								'<span class="js-hidden" id="exe-client-search-text-' +
								i +
								'">';
							n += end;
							n += "</span>";
				...
				$(".exe-client-search-result-link", $exe.clientSearch.results).click(
					function () {
						var extra = "";
				...
				$exe.clientSearch.results.html(
					"<p>" +
						$exe_i18n.noSearchResults.replace(
							"%",
							"<strong>" + query + "</strong>"
						) +
						"</p>"
	...
	setModalWindowContentSize: function () {
		if (window.chrome) {
			$(".exe-dialog-text img").each(function () {
				var e = $(this);
				var h = e.attr("height");
				var w = e.attr("width");
		...
		if (typeof $.prettyPhoto != "undefined") {
			...
			lightboxLinks.each(function (i) {
				...
				if (isAudio || isVideo) {
					...
					$(this).attr("href", "#" + id);
					var hiddenPlayer = $(
						'<div class="exe-media-box js-hidden" id="' + id + '"></div>'
					);
					if (isAudio)
						hiddenPlayer.html(
							'<div class="exe-media-audio-box"><audio controls="controls" src="' +
								ref +
								'" class="exe-media-box-element exe-media-box-audio"><a href="' +
								ref +
								'">audio/mpeg</a></audio></div>'
						);
					else
						hiddenPlayer.html(
							'<div class="exe-media-video-box"><video width="480" height="385" controls="controls" class="exe-media-box-element"><source src="' +
								ref +
								'" /></video></div>'
						);
				...
				var t = this.title;
				if (ref.indexOf("#") == 0 && $(ref).length == 1 && t && t != "")
					$(ref).prepend('<h2 class="pp_title">' + t + "</h2>");
			});
			lightboxLinks.prettyPhoto({
				...
				changepicturecallback: function () {
					...
					if (media.length == 1 && media[0].hasAttribute("src")) {
						if (media.hasClass("exe-media-box-audio"))
							cont.attr("class", "pp_content_container with-audio");
						var src = media.attr("src");
						var ext = src.split("/");
						ext = ext[ext.length - 1];
						ext = ext.split(".")[1];
						$(".pp_details .pp_description").append(
							' <span class="exe-media-download"><a href="' +
								src +
								'" title="' +
								$exe_i18n.download +
								'" download>' +
			...
			if (
				lightboxLinks.length == 0 &&
				eXeGalleries.length > 0 &&
				typeof exe_editor_mode == "undefined"
			) {
						...
						if (ul.length == 1 && ul.attr("id") != "") {
							if ($("#" + ul.attr("id") + "-warning").length == 0) {
								...
								if ($("body").hasClass("exe-epub3"))
									txt += "<br /><br />" + $exe_i18n.epubJSerror;
								ul.prepend(
									'<div id="' + ul.attr("id") + '-warning">' + txt + "</div>"
								);
							}
						}
					};
			...
			for (var n = 0; n < r.length; n++) {
				r[n].onfocus = function () {
					...
					this.parentNode.className +=
						(this.parentNode.className.length > 0 ? " " : "") + "sfhover";
					if (this.parentNode.parentNode.parentNode.nodeName == "LI") {
						this.parentNode.parentNode.parentNode.className +=
							(this.parentNode.parentNode.parentNode.className.length > 0
								? " "
								: "") + "sfhover";
						if (
				...
				r[n].onblur = function () {
					...
					if (this.parentNode.parentNode.parentNode.nodeName == "LI") {
						this.parentNode.parentNode.parentNode.className =
							this.parentNode.parentNode.parentNode.className.replace(
								new RegExp("( ?|^)sfhover\\b"),
								""
							);
						if (
							...
							this.parentNode.parentNode.parentNode.parentNode.parentNode.className =
								this.parentNode.parentNode.parentNode.parentNode.parentNode.className.replace(
									new RegExp("( ?|^)sfhover\\b"),
									""
								);
						}
	...
	mediaReplace: function () {
		if (navigator.appName == "Microsoft Internet Explorer") {
			...
			while (t--) {
				if (
					e[t].type == "video/quicktime" ||
					e[t].type == "audio/x-pn-realaudio-plugin"
				) {
					if (typeof e.classid == "undefined") {
						e[t].style.display = "none";
						var n = "02BF25D5-8C17-4B23-BC80-D3488ABDDC6B";
						if (e[t].type == "audio/x-pn-realaudio-plugin")
							n = "CFCDAA03-8BE4-11CF-B84B-0020AFBBCCFA";
						var r = e[t].height;
						var i = e[t].width;
						var s = e[t].data;
						...
						o.innerHTML =
							...
							'" height="' +
							r +
							'"><param name="controller" value="true" /><param name="src" value="' +
							s +
		...
		init: function () {
			...
			l.each(function (i) {
				...
				$("dt", e).each(function () {
					...
					h = $(t).html();
					$(t).html(
						"<a href='#' class='exe-dd-toggler exe-dl-" +
							i +
							"-a'><span class='icon'" +
							s +
							">» </span>" +
	...
	math: {
		...
		createLinks: function (math) {
			...
			math.each(function () {
				...
				var html = "";
				if (img.length == 1)
					html += '<a href="' + img.attr("src") + '" target="_blank">GIF</a>';
				if (!mathjax) {
		...
		showCode: function (e) {
			...
			html +=
				'<style type="text/css">body{font:10pt/1.5 Verdana,Arial,Helvetica,sans-serif;margin:10pt;padding:0}</style>';
				...
				if (r.length == 1 && i.length == 1) {
					...
					s.html(
						'<a href="#' +
							...
							'" title="' +
							$exe_i18n.show +
							'" class="hint-toggler show-hint" id="toggle-' +
							n +
	...
	iDeviceToggler: {
		init: function () {
			...
			t.each(function () {
				...
				if (eP.length) {
					var n =
						'<p class="toggle-idevice toggle-' +
						c +
						'"><a href="#" id="toggle-idevice-' +
						eP.attr("id") +
						"-" +
						c +
						'" title="' +
	...
	alignMediaElement: function (e) {
		...
		if (typeof r == "string" && r.indexOf("mejs-container") == 0) {
			...
			if (i == "auto" && i == s)
				$(n).wrap('<div style="text-align:center"></div>');
		}
	},
	loadMediaPlayer: {
		isCalledInBox: false,
		...
		getPlayer: function () {
			...
			$exe.mediaelements
				.each(function () {
					if (
						typeof this.localName != "undefined" &&
		...
		init: function () {
			if (typeof eXe != "undefined") {
				mejs.MediaElementDefaults.flashName =
					"../scripts/mediaelement/" + mejs.MediaElementDefaults.flashName;
				mejs.MediaElementDefaults.silverlightName =
			...
			if (!$exe.loadMediaPlayer.isCalledInBox)
				$("#pp_full_res .exe-media-box-element").mediaelementplayer();
		},
	},
			...
			$("iframe").each(function () {
				...
				if (typeof s == "string") {
					...
					if (!i.hasClass("external-iframe") && s.indexOf("http") == 0) {
						i.addClass("external-iframe").before(
							"<span class='external-iframe-src' style='display:none'><a href='" +
								s +
								"'>" +
								s +
								"</a></span>"
	...
	insertAtCursor: function (e, t, n) {
		if (e.selectionStart || e.selectionStart == "0") {
			...
			e.value =
				e.value.substring(0, r) + t + e.value.substring(i, e.value.length);
...
$exe.cloze = {
	...
	change: function (ele) {
		...
		var instant = eval(
			document.getElementById("clozeFlag" + ident + ".instantMarking").value
		);
		if (instant) {
	...
	checkWordAnswer: function (ele, original_answer) {
		...
		var ident = $exe.cloze.getIds(ele)[0];
		var strictMarking = eval(
			document.getElementById("clozeFlag" + ident + ".strictMarking").value
		);
		var checkCaps = eval(
			document.getElementById("clozeFlag" + ident + ".checkCaps").value
		);
				...
				while (string1) {
					misses = Math.floor(
						(Math.abs(string1.length - string2.length) +
							Math.abs(guess.length - answer.length)) /
							2
					);
	...
	onLangChange: function (ele) {
		...
		var instant = eval(
			document.getElementById("clozelangFlag" + ident + ".instantMarking").value
		);
		if (instant) {
	...
	checkLangWord: function (ele) {
		...
		var ident = $exe.cloze.getLangIds(ele)[0];
		var strictMarking = eval(
			document.getElementById("clozelangFlag" + ident + ".strictMarking").value
		);
		var checkCaps = eval(
			document.getElementById("clozelangFlag" + ident + ".checkCaps").value
		);
		if (!checkCaps) {
			guess = guess.toLowerCase();
	...
	showLangScore: function (ident, mark) {
		var showScore = eval(
			document.getElementById("clozelangFlag" + ident + ".showScore").value
		);
		if (showScore) {
			var score = 0;
			...
			var scorePara = document.getElementById("clozelangScore" + ident);
			scorePara.innerHTML =
				$exe_i18n.yourScoreIs + score + "/" + inputs.length + ".";
		}
	},
