...
function ContentServers() {
	const [proxies, setProxies] = useState({ results: [] });
	...
	useEffect(() => {
		...
		axios
			.get(
				"https://shadowmere.akiel.dev/api/proxies/?format=json&is_active=true&page=1"
			)
			.then((response) => {
	...
	// }else {
        return (
            //     <div class="loader-screen w-full h-full bg-white dark:bg-[#141414] flex items-center justify-center">
            //      <lottie-player src="./others/logo-animation-2-dark.json" background="transparent" speed="1" style="width: 100px; height: 100px;" loop autoplay></lottie-player>
            //     </div>
            ...
            proxies.results.map((item) => (
                <div className="item-server-vpn w-full h-20 sm:h-24 bg-white dark:bg-[#212121] dark:text-[#cfcfcf] rounded-lg shrink-0 flex items-center justify-between px-4 md:px-8 py-2 shadow-lg hover:cursor-pointer hover:-translate-y-[2px] dark:hover:bg-[#303030] hover:bg-[#FDFDFD] transition-all">
                    <div className="data-country-ip gap-2 md:gap-4 flex items-center">
                        <div className="font-twemoji select-none flex items-center text-xl w-6 h-6 cursor-pointer">
                            {getFlagEmoji(item.location_country_code)}
                        </div>
                        <span className="w-12 h-6 flex items-center justify-center select-none bg-[#303030] dark:bg-[#cfcfcf] px-2 text-[#cfcfcf] dark:text-[#303030] font-medium text-xs sm:text-sm rounded">
                            {getPercentage(item.times_check_succeeded, item.times_checked)}%
                        </span>
                        <span className="font-semibold w-28 overflow-hidden truncate text-sm sm:text-base select-none">
                            {item.ip_address}
                        </span>
                    </div>
                    ...
                    <div className="data-operations gap-4 flex flex-row-reverse items-center">
                        <button
                            ...
                            className="opt-copiar relative w-4 h-4 sm:w-10 sm:h-10 rounded-md flex items-center justify-center cursor-pointer hover:bg-[#EBEBEB] dark:hover:bg-[#1B1B1B] active:bg-[#D7D7D7] dark:active:bg-[#111111] transition-colors transition-transform active:scale-[95%]">
                            <div className="absolute w-4 h-4 sm:w-10 sm:h-10 rounded-md flex items-center justify-center">
                                <svg
                                    className=" bg-transparent w-6 h-6 stroke-[#303030] dark:stroke-[#cfcfcf]"
                                    viewBox="0 0 24 24"
                                    fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M5 11.2L10.25 16L19 8"
                                        stroke-width="3"
            ...
            // <div>{proxies.results.map(p => (<div>
            //     {p.id}
            //     {getFlagEmoji(p.location_country_code)}
            // </div>))}</div>
        );
